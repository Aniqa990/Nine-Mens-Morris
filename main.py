...
BOARD_COLOR = (222, 184, 135)  # Wooden board color
LINE_COLOR = (0, 0, 0)
...
FPS = 60
PIECE_RADIUS = 15
ANIMATION_SPEED = 0.05  # Slower animation for better visibility
...
# Board positions (x, y coordinates for each point)
board_positions = [
...
def heuristicFunction(board, isStage1):
	...
	potentialMillsPlayer2 = opponentPotentialMillCount(board, "2") #enemy mills

	if not isStage1:
		movablePiecesPlayer1 = len(NextMoves(board))
		if player1Pieces <= 2 or movablePiecesPlayer1 == 0:
			utility = float('inf')
		elif player2Pieces <= 2:
			utility = float('-inf')
		else:
			if (player2Pieces < 4):
				utility += 100 * numPossibleMillsPlayer1
				utility += 200 * potentialMillsPlayer2
			else:
				utility += 200 * numPossibleMillsPlayer1
				utility += 100 * potentialMillsPlayer2
			utility -= 25 * movablePiecesPlayer1
			utility += 50 * (player2Pieces - player1Pieces)
	else:
		if player2Pieces < 4:
			utility += 100 * numPossibleMillsPlayer1
			utility += 200 * potentialMillsPlayer2
...
def ai_turn():
    ...
    try:
        if game_state.phase == 1:
            ...
            evalBoard = alphaBetaPruning(game_state.board.copy(), game_state.depth, False, 
                                      game_state.alpha, game_state.beta, True, heuristicFunction)
            
            game_state.board = evalBoard.board
            print("Board length:", len(game_state.board))


        # Update piece counts manually (optional: you can also recalculate)
        game_state.ai_pieces_to_place = 0 if game_state.phase == 1 else game_state.ai_pieces_to_place
        game_state.human_pieces_on_board = sum(1 for p in game_state.board if p == "1")
        game_state.ai_pieces_on_board = sum(1 for p in game_state.board if p == "2")

        # Transition phase if needed
        if game_state.phase == 1 and game_state.human_pieces_to_place == 0 and game_state.ai_pieces_to_place == 0:
            game_state.phase = 2
            game_state.message = "Move your piece (White)"
        
        else:  # Phase 2-3: Moving or Flying
            eval_board = alphaBetaPruning(game_state.board.copy(), game_state.ai_depth, False,
...
def ai_turn():
    ...
    try:
        ...
        if game_state.phase == 1:
            evalBoard = alphaBetaPruning(original_board.copy(), game_state.depth, False, 
                                      game_state.alpha, game_state.beta, True, heuristicFunction)
            game_state.board = evalBoard.board
            ...
            eval_board = alphaBetaPruning(original_board.copy(), game_state.ai_depth, False,
                                      game_state.alpha, game_state.beta, False, heuristicFunction)
            game_state.board = eval_board.board
...
def check_game_over():
    if game_state.phase == 2 or game_state.phase == 3:
        if game_state.human_pieces_on_board < 3:
            game_state.message = "Game Over - You Lost!"
            game_state.game_over = True
            return True
        if game_state.ai_pieces_on_board < 3:
