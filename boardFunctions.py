...
def isCloseMill(position, board):
	...
	if (player != "X"):
		return potentialMill(position, board, player)
...
def NextMoves(board):
	if (countPieces(board, "1") == 3) or (countPieces(board, "2") == 3):
...
def getPossibleMillCount(board, player):
	...
	for i in range(len(board)):
		if (board[i] == "X"):
			if potentialMill(i, board, player):
				count += 1
	return count

def getEvaluationStage23(board):
	numWhitePieces = countPieces(board, "1")
	numBlackPieces = countPieces(board, "2")
	mills = getPossibleMillCount(board, "1")

	evaluation = 0

	board_list = NextMoves(board)
...
def potentialMillCount(position, board, player):
	adjacent_list = adjacentPositions(position)

	for i in adjacent_list:
		if (board[i] == player) and (not potentialMill(position, board, player)):
			return True
	return False

def opponentPotentialMillCount(board, player):
	count = 0
		...
		if (board[i] == player):
			...
			for pos in adjacent_list:
				if (player == "1"):
					...
					if (board[pos] == "1" and potentialMillCount(pos, board, "1")):
						count += 1
	return count
